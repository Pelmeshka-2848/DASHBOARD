...
def update_graphs(relayout_data1, relayout_data2, relayout_data3, relayout_data4, relayout_data5):
    try:
        ...
        fig3 = px.pie(df, names='Год выпуска', title='Круговой график').update_layout(
            margin=dict(l=20, r=20, t=40, b=20),
            font=dict(family="Arial, sans-serif", size=12, color="Black"),
            paper_bgcolor="White",
        )

        fig4 = px.line(df, x='Дата', y='Время мойки', title='Линейный график').update_layout(
            margin=dict(l=20, r=20, t=40, b=20),
            font=dict(family="Arial, sans-serif", size=12, color="Black"),
            paper_bgcolor="White",
        )

        fig5 = px.box(df, x='Цена', y='Оценка', title='Ящичковый график').update_layout(
            margin=dict(l=20, r=20, t=40, b=20),
            font=dict(family="Arial, sans-serif", size=12, color="Black"),
            paper_bgcolor="White",
        )
